...
plot_inversion_dotplot <- function(sp_df, species_name,title) {
  sp_df<-sp_df %>%
    filter(Species == species_name)
  # determine plot limits
  lims <- range(c(sp_df$RefStart, sp_df$RefEnd))
  ...
  ggplot(sp_df) +
    ...
    labs(
      ...
      y = "Reference position (bp)"
    ) +
    
    theme_minimal(base_size = 13)
}

plot_ac<-plot_inversion_dotplot(support_df,"A.coerulescensAC","AC")
plot_ai<-plot_inversion_dotplot(support_df,"A.insularisAI","AI")
plot_aw<-plot_inversion_dotplot(support_df,"A.woodhouseiiAW","AW")
plot_hf<-plot_inversion_dotplot(support_df,"house_finches","House Finches")
